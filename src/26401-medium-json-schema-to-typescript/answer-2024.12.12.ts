...
type JSONSchema2TS<T> =
  ...
  ? T extends { enum: infer Enum extends string[] }
    ? Enum[number]
    : string
  : T extends { type: 'number' }
    ? T extends {  enum: infer Enum extends number[] }
      ? Enum[number]
      : number
      ...
      : T extends { type: 'object' }
        ? T extends { properties: infer Properties extends Record<string, unknown> }
          ? T extends { required: infer Required extends string[] }
            ? Omit<
              { [K in keyof Properties as K extends Required[number] ? K : never]: JSONSchema2TS<Properties[K]> } &
              { [K in keyof Properties as K extends Required[number] ? never : K]?: JSONSchema2TS<Properties[K]> }, 
              never
            >
            : { [K in keyof Properties]?: JSONSchema2TS<Properties[K]> }
          : Record<string, unknown>
        : T extends { type: 'array' }
          ? T extends { items: infer Items extends Record<string, unknown> }
            ? JSONSchema2TS<Items>[]
            : unknown[]
          : unknown[]
